...
public class SegTree {
	public static void main(String args[]) {
		...
		test.updateRange(1,3, -10);
		System.out.println();
		System.out.println(test.rmq(1, 3));
		System.out.println(test.rsq(1, 3));
		System.out.println(test.rsq(4, 6));
		System.out.println(test.rsq(3, 5));
		for (int item : test.st)
			System.out.print(item + " ");
		System.out.println();
		for (int item : test.sq)
			System.out.print(item + " ");
		System.out.println();
		for (int item : test.A)
			System.out.print(item + " ");
	...
	public static class SegmentTree {
		...
		public void updateRange(int node, int start, int end, int l, int r, int val) {
			...
			int mid = (start + end) / 2;
			updateRange(left(node), start, mid, l, r, val);
			updateRange(right(node), mid + 1, end, l, r, val);
			// Use the result of children calls to update this node
			sq[node] = sq[right(node)] + sq[left(node)];
			st[node] = (A[st[right(node)]]<=A[st[left(node)]]) ? st[right(node)] : st[left(node)];  
		}
